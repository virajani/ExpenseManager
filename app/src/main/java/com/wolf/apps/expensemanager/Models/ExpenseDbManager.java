...
public class ExpenseDbManager extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db){
        String tbl_account = "CREATE TABLE transaction_account(ac_id INTEGER PRIMARY KEY AUTOINCREMENT, ac_name TEXT, ac_type INTEGER, amount NUMERIC)";
        ...
        String tbl_income_details = "CREATE TABLE income_details( i_details_id INTEGER PRIMARY KEY AUTOINCREMENT, i_details_amount NUMERIC, i_details_description TEXT, date BIGINT, i_sub_cat_id INTEGER, ac_id INTEGER, FOREIGN KEY(i_sub_cat_id) REFERENCES income_sub_category(i_sub_cat_id), FOREIGN KEY(ac_id) REFERENCES account(ac_id))";
        ...
        String tbl_expense_details = "CREATE TABLE expense_details( e_details_id INTEGER PRIMARY KEY AUTOINCREMENT, e_details_amount NUMERIC, e_details_description TEXT, date BIGINT, e_sub_cat_id INTEGER, ac_id INTEGER, FOREIGN KEY(e_sub_cat_id) REFERENCES expense_sub_category(e_sub_cat_id), FOREIGN KEY(ac_id) REFERENCES account(ac_id))";

        db.execSQL(tbl_account);
